...
\IEEEcompsocitemizethanks{\IEEEcompsocthanksitem J. Nam and L. Tan is with the Department
...
\IEEEcompsocthanksitem S. Kim is with the Department of Computer Science and Engineering, the Hong Kong University of Science and Technology, Hong Kong, China.
\IEEEcompsocthanksitem W. Fu and T. Menzies are with the Department of Computer Science, NC State University, Raleigh, NC, 27695.}% <-this % stops an unwanted space
...
software engineering. We can build a prediction model with past defect data
collected from a software project and predict new defects in the same
...
be feasible. However, there are two major limitations to transfer knowledge across projects for CPDP. First, CPDP requires projects that have the same metric set,
...
projects with heterogeneous metric sets. Second, it is not well-studied when we can conduct effective\lin{when it is effective to conduct} CPDP. In other words, since a target dataset have few data samples as its history is short, it is important to investigate the lower bounds of data samples\lin{data sample sizes?} for effective CPDP.

To address the first limitation, we propose heterogeneous defect
...
statistical significance. For the second limitation, we conduct additional empirical study by using various sampling sizes of source and target datasets and validate the generality of its results through a {\em Monte Carlo} simulation. Our results suggest that the sample size of 200 for source (with at least 20 defective samples) and target datasets could be effective enough for our HDP models.
\end{abstract}
