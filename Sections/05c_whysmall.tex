...
of defect data  into that model; \lin{and} 
\item Identifies what would\lin{delete} need to change before the above results would\lin{delete} no longer hold.
\ei
...
that a small number
of examples are sufficient to build a defect predictor, even when the data is being\lin{delete} transferred from columns with other names.
In the following we will build a model computing\lin{to compute or that computes} the probability that $n$ training examples are sufficient to detect $e$\% defective modules.

In order to simplify the analysis, we will\lin{delete} divide $n$ \lin{What is n? Is it N?} into
  $n<50, n<100, n<200,n\ge 200$ \lin{Is it $50\le n<100$ or simply $n<100$?} respectively (and note that $n  \ge 200$ is where the above
  results do not hold).
...
i.e.\lin{i.e.,} they are divided into $b=2$ bins \lin{I don't quite understand this. Expand the explanation?}.
...
(and by ``need'' we mean that adding in\lin{delete} unnecessary dimensions does not add to\lin{improve} the accuracy
...
\subsubsection{A Maths\lin{Mathematical} Model}

Before writing down some maths, it is useful to start with some intuitions\lin{The intuition of our mathematical model is as follows.}.
...
For completeness, it should be added  that\lin{delete it ... that} the procedure of the above paragraph is an {\em upper bound} on the
...
\subsubsection{Monte Carlo Simulation}
    The above maths lets\lin{let, please check grammar carefully} us define
    a  Monte Carlo simulation to assess the external validity of our results.
    \lin{broken sentences} 1000 times, we picked $k,d,b,e$ values at random from:
    \bi
      \item $k \in \{1,2,3,4,5\}$;
  ...
  \item $b \in \{2,3,4,5,6,7\}$ bins; 
    \item $e\in \{0.1,0.2,0.3,0.4\}$
     ...
     \lin{a broken paragraph} 1000 times,
...
likely to find defects occuring\lin{occurring, please spell check} at probability $e$.}\label{fig:dtree}
\end{figure}
\subsubsection{Decision Tree Learning}
    ...
    ranges of $\{k,b,d,e\}$. Decision tree learners seek an attribute range that, when used to split the data,
      ...
      The decision tree learner is then called recursively on each split. \lin{This experiment is unclear to me. What is the training set and test set? What are the labels and features? Judging from the tree, I know what the features are, but it should be expliclity stated.}
...
\lin{It may be a good idea to explain how to interpret the tree. It is intuitive to me but it may not be to someone who is not familiar with decision tree.}
...
data being explored; \lin{the following text doesn't connect with this bullet point. Judging from the text above, the following should be a feature, but I don't see how "how many dimensions are needed" is a feature. This is somewhat related to the lack of explanation of the decision tree learning I mentioned earlier.} specifically, how many dimensions are needed and how many bins are required when dividing
those dimensions.
\ei
