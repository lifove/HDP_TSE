...
To systematically evaluate heterogeneous defect prediction (HDP) models, we set
two research questions.
\squishlist
  \item RQ1: Is heterogeneous defect prediction comparable to WPDP
  , CPDP using common metrics (CPDP-CM), and CPDP-IFS?
...
(Baseline2), and CDDP-IFS (Baseline3)~\cite{He14}. In Section~\ref{sec:Result}, we report HDP results compared to three baselines as well as its performances in different settings such as various metric selection approaches, matching analyzers, and machine learners. In RQ2, we investigate the lower bounds of the effective sizes of source and target datasets for HDP models. Since HDP compares the distributions of source metrics to those of target metrics, it is important to seek the empirical evidence for the effective size of source and target datasets to match source and target metrics. We report the results of the empirical study for RQ2 in Section~\ref{sec:sizelimit} and validate the generality of its results in Section~\ref{sect:xplain}.
%In RQ1, we validate
...
%\scriptsize
\centering
\caption{The 28 defect datasets from five groups.}
\label{tab:datasets}
...
\multirow{5}{*}{\specialcell{NASA\\\cite{promise12,Shepperd13}}}
	&	cm1
	&\centering327
	&	42(12.84\%)
	& \multirow{5}{*}{37}
...
\subsection{Matching Score Cutoff Thresholds}
...
The 16 distribution characteristics are used as features to build a prediction model.
...
To measure the effect size of AUC results among baselines and HDP, we compute Cliff's $\delta$ that is a non-parametric effect size measure~\cite{romano06}. Romano et al. suggested to evaluate the magnitude of the effect size as follows: negligible ($|\delta|<$ 0.147), small ($|\delta|<$ 0.33), medium ($|\delta|<$ 0.474), and large (0.474 $\leq|\delta|$)~\cite{romano06}.
...
a baseline outperforms that of our HDP approach with statistical
significance.
