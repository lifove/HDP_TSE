...
Like prior works\lin{work, not countable} \cite{nam2013transfer,
...
we  add a third initial sampling process before doing\lin{performing} metric matching:
...
included. With those sampled data, we do\lin{perform} metric matching to build learner\lin{learners or a learner}
and finally predict labels of the original all data\lin{all original data} in \lin{the} target project.
...
\lin{It's unclear to me. So metric matching on the 50, 100, etc.? What is third initial sampling? Add a backward reference to it? One caveat: I focused on reading the abstract, Section 3, 7, and 8, and I only skimmed over the rest of the paper, so maybe I missed it. But in general it is still good to add a backward reference.}
...
\item
  The {\em colourful}\lin{coloured?} lines show results of transferring from {\em some} small $N$ number of samples (instead of {\em all})
...
HDP framework to get\lin{obtain} a good predictor.
...
% 	\vspace{0.5mm}
	...
	\caption{Improvements of using sampled data N \lin{Earlier N was used to refer to the number of reference projects. Choose different variables for different concepts. } = \{50, 100, 150, 200\} with EPV constraints. We label the data in table \ref{tab:datasets} from a to z, and the last two data sets ar5 and ar6 as A and B.}
	\label{fig:small_epv}
\end{figure}
...
\bi
\item Evidence that this ``a few examples are enough'' has been seen in other domains; \lin{add citations}
\item Methods to reduce the cost of reducing this data set, even further. \lin{this phrase is awkward and difficult to understand, rephrase}
  \ei
Working in the domain cardiology,
  ...
  was very small: usually just 2. Hence, applying  Peduzzi et al.'s rule, we picked $2*10=20$ \lin{why *10?}
  randomly selected defective data, then randomly added $(N-M)$ non-defective data for
  $N\in \{50,100,150,250\}$ for the source data. For the target data set, since we don't\lin{do not}
  know the labels of data\lin{data labels}, we simply sampled $N$ for target data in metric matching.
...
\item
  ...
  does not change by very\lin{delete the weasel word very} much. Also, if we compare $N=200$ between \fig{small_data}
  and \fig{small_epv}, there seems\lin{what does "seems" mean exactly? not very scientific here} no large disadvantage of using just
  $M=20$ defective modules.

  Note that such a very small sample would be very\lin{delete the weasel word very} quick to collect: after writing (say) a few
