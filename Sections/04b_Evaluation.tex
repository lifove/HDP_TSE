...
proposed above uses {\em all} the instances in feasible data
sets as training data to perform KS-test and build
defect prediction learners.
Collecting {\em all} that data requires waiting for a project to finish before
transferring its learned lessons. This begs the question ``how early can we transfer?''.
That is, how {\em few} source data examples do we need before transfer can be effective?
...
we  add a third initial sampling process:
instead of using all the instances from
candidate data sets, that  data sets will
...
The results for this HDP-with-limited-training-data experiment is shown in \fig{small_data}
...
\item
  The {\em red} line shows results from transferring from {\em some} small $N$ number of samples
  in the source data.
\item
...
training examples are enough for effective transfer learning. Naturally,
...
Working in the domain cardiology,
  ...
  that study would predict that  10 defective modules per independent variable should
  suffice for effective learning. Of course, learners require defective and {\em non-defective}
  examples so to $M$ defective examples, we add will add $N > M$ more.
  ...
  randomly selected defect modules, then added up to $N$ more for $N\in \{50,100,150,250\}$.
...
\item
  The {\em black/red} lines show the results using {\em all/some} of the data, respectively
