...
% 	\vspace{0.5mm}
	...
	\caption{Improvements of using sampled data over all data with sampled size N = \{50, 100, 150, 200\}. We label the data in table \ref{tab:datasets} from a to z, and continue from A to H, the last two datasets ar5 and ar6 as G and H.}
...
defect prediction learners, which takes several days with a machine with 24 cores in \cite{Nam15HDP}.
Such a long runtime can prevent HDP from applying into practice.
...
% transferring its learned lessons. 
This begs the question  ``Can we use a few, instead of {\em all}, source and target project data to apply HDP?'' In this section, we conduct an empirical study to answer these questions related to RQ2.

% To investigate the size limits for effective transfer learning in the setting of CPDP across datasets with heterogeneous metric sets, we focus on the HDP approach. There are other approaches such as CPDP-IFS~\cite{He14} and CCA+~\cite{Jing15}. 
% In Section~\ref{sec:Result}, we observed that HDP outperforms CPDP-IFS. In addition, CCA+ was evaluated in somewhat different context, i.e., cross-company defect prediction and with 14 projects which are far less than 28 projects used in our experiments for HDP. In addition, the implementation of CCA+ is not publicly available yet and more complex than HDP. For this reason, we conducted our empirical study for RQ2 based on HDP.
...
% 	\vspace{0.5mm}
	...
	\caption{Improvements of using sampled data N  = \{50, 100, 150, 200\} with EPV constraints. We label the data in table \ref{tab:datasets} from a to z, and continue from A to H, the last two datasets ar5 and ar6 as G and H.}
	\label{fig:small_epv}
\end{figure*}
