...
To investigate the size limits for effective transfer learning in the setting of CPDP across datasets with heterogeneous metric sets, we focus on the HDP approach. There are other approaches such as CPDP-IFS~\cite{He14} and CCA+~\cite{Jing15}. However, in Section~\ref{sec:Result}, we observed that HDP outperforms CPDP-IFS. CCA+ was evaluated in somewhat different context, i.e., cross-company defect prediction and with 14 projects which are far less than 28 projects used in our experiments for HDP. In addition, the implementation of CCA+ is not publicly available yet and more complex than HDP. For this reason, we conducted our experiments for RQ2 based on HDP.
...
Working in the domain cardiology,
  ...
  was very small: usually just 2. Hence, applying  Peduzzi et al.'s rule (EPV=10), we picked $2*10=20$ %\lin{why *10?}\wei{EPV=10, mentioned in the last paragraph. }
